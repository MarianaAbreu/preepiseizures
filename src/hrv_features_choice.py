...
hrv_features_dict = {'hr_min': False, 'hr_max': False, 'hr_minmax': False, 'hr_mean': False, 'hr_median': False, 
                'rr_min': False, 'rr_max': False, 'rr_minmax': False, 'rr_mean': False, 'rr_median': False, 
                'rmssd': False, 'nn50': False, 'pnn50': False, 'sdnn': False, 'hti': False, 'tinn': False,
                
                'ulf_peak': False, 'ulf_pwr': False, 'ulf_rpwr': False, 'vlf_peak': False, 'vlf_pwr': False,
                'vlf_rpwr': False, 'lf_peak': False, 'lf_pwr': False, 'lf_rpwr': False, 'hf_peak': False, 'hf_pwr': False,
                'hf_rpwr': False, 'vhf_peak': False, 'vhf_pwr': False, 'vhf_rpwr': False, 'lf_hf': False, 'lf_nu': False,
                'hf_nu': False, 'total_pwr': False,}


class HRV():

    def __init__(self, rri, sampling_rate):
        self.rri = rri
...
def get_hr_features(rri, sampling_rate=1000.):
    """
    """
    features_dict = {}
    hr_signal = 60 / (rri / sampling_rate)
    features_dict['hr_min'] = np.min(hr_signal)
    features_dict['hr_max'] = np.max(hr_signal)
    features_dict['hr_minmax'] = features_dict['hr_max'] - features_dict['hr_min']
    # features_dict['hr_mean'] = np.mean(hr_signal)
    # features_dict['hr_median'] = np.median(hr_signal)

    return features_dict


def get_hrv_time_domain_features(rri, sampling_rate=1000.):
    """
    """
    features_dict = {}
    ...
    features_dict['rr_min'] = np.min(rri)
    features_dict['rr_max'] = np.max(rri)
    features_dict['rr_minmax'] = features_dict['rr_max'] - features_dict['rr_min']
    features_dict['rr_mean'] = np.mean(rri)
    features_dict['rr_median'] = np.median(rri)
    features_dict['rmssd'] = (rri_diff ** 2).mean() ** 0.5
...
def hrv(rpeaks=None, sampling_rate=1000., rri=None, parameters='auto',
    ...
    assert rpeaks is not None or rri is not None, "Please specify an R-Peak or RRI list or array."
    ...
    assert parameters in parameters_list, f"'{parameters}' is not an available input. Enter one from: {parameters_list}."
    ...
    if parameters in ['time', 'auto', 'all']:
        try:
            ...
            warning('WARNING: Time-domain features not computed. Check input.')
            print(e)
    ...
    if parameters in ['frequency', 'auto', 'all']:
        try:
            ...
            warning('WARNING: Frequency-domain features not computed. Check input.')
            print(e)
            pass
    ...
    if parameters in ['non-linear', 'auto', 'all']:
        try:
            ...
            warning('WARNING: Non-linear features not computed. Check input.')
            print(e)
            pass
...
def rri_correction(rri=None, threshold=250):
    ...
    assert rri is not None, "Please specify an RRI list or array."

    # ensure input format
    rri = np.array(rri, dtype=float)

    # compute local average
    rri_filt, _ = st.smoother(signal=rri, kernel='median', size=15)
    ...
    rri[artifacts] = interp1d(np.where(~artifacts)[0], rri[~artifacts],
                              kind='cubic', fill_value='extrapolate')(np.where(artifacts)[0])
...
def hrv_timedomain(rri, duration=None, detrend_rri=True, show=False, **kwargs):
    ...
    if detrend_rri:
        ...
        Warning('Time domain: the rri sequence was detrended.')
    else:
        rri_det = rri
    ...
    #                     "compute time-domain features.")
    assert duration >= 10, "Signal duration must be greater than 10 seconds to compute time-domain features."
    ...
    if duration >= 10:
        ...
        out = out.append([hr_min, hr_max, hr_minmax],
                         ['hr_min', 'hr_max', 'hr_minmax'])
...
def hrv_frequencydomain(rri=None, duration=None, freq_method='FFT',
    ...
    assert freq_method in freq_methods, f"'{freq_method}' is not an available input. Choose one from: {freq_methods}."
    ...
    #                     "compute frequency-domain features.")
    assert duration >= 20, "Signal duration must be greater than 20 seconds to compute frequency-domain features."
    ...
    if detrend_rri:
        ...
        Warning('Frequency domain: the rri sequence was detrended.')
    ...
    if duration >= 20:
        ...
        fb_out = compute_fbands(frequencies=frequencies, fbands=fbands, powers=powers, show=False)
...
def hrv_nonlinear(rri=None, duration=None, detrend_rri=True, show=False, **kwargs):
    ...
    #                     "compute non-linear features.")
    assert duration >= 90, "Signal duration must be greater than 90 seconds to compute non-linear features."
    ...
    if detrend_rri:
        if 'rri_detrended' in kwargs:
            ...
            rri_det = detrend_window(rri)['rri_det']
        Warning('Non-linear domain: the rri sequence was detrended.')
    else:
        rri_det = rri
...
def compute_fbands(frequencies, powers, fbands=None, method_name=None,
    ...
    for fband in fbands.keys():
        ...
        out = out.append([pwr, rpwr], [fband + '_pwr',
                                             fband + '_rpwr'])
